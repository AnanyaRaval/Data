...
cp = cp.append({'Command':'cp -u * ananya','NL Queries':['Copy all files in this folder to ananya only if they are not present in ananya.',
					'Update folder ananya by copying files from this directory. Update only if file not present in ananya.']},ignore_index=True)
...
cp = cp.append({'Command':'cp -n instructions.txt backup','NL Queries':['Create a copy of instructions.txt named backup. Copy only if backup not present in current folder.',
								'How do I copy instructions.txt to backup without overwriting it.']},ignore_index=True)
...
cp = cp.append({'Command':'cp file1.txt file2.txt dir1','NL Queries':['How do I copy file1.txt and file2.txt to directory dir1?',
					'How do I copy two files file1.txt and file2.txt at the same time?',
						'Copy two files file1.txt and file2.txt to a given directory.']},ignore_index=True)

cp = cp.append({'Command':'cp -i game1.txt dir1','NL Queries':['How do I copy file game1.txt to dir1 with a prompt?',
					'How do I ensure that the file game.txt get copied only after asking me?',
...
cp = cp.append({'Command':'cp -r dir1 dir2','NL Queries':['Copy entire directory structure dir1 to dir2.',
				'How do I copy the files and folders inside dir1 recursively into dir2?',
					'Copy all files and folders inside file to file2.',
						'Command to copy files and folders to dir1 recursively.']},ignore_index=True)
...
cp = cp.append({'Command':'cp -v joke1 joke2','NL Queries':['How do I copy joke1 to joke2 along with details on what is being done?',
			'Copy joke1 to joke2 and get information on what is being done.',
				'How do I know whether joke1 is being coped to joke2?']},ignore_index=True)
...
cp = cp.append({'Command':'cp --help','NL Queries':['How to get help with information on cp command?',
		'Display all the option available with cp command.',
			'List all the flags available with cp command.','Where can I find help with cp command?']},ignore_index=True)
...
cp = cp.append({'Command':'cp ../file1.txt .','NL Queries':['How do I copy a file from from parent directory to the current folder?',
...
cp = cp.append({'Command':'cp new1.txt /','NL Queries':['How do I copy new1.txt to the root directory?',
				'Copies new1.txt to root directory.','How do I copy file new1.txt to root folder?']},ignore_index=True)

cp = cp.append({'Command':'cp best.txt ~','NL Queries':['How do I copy best.txt to the home directory?',
				'Copies best.txt to home directory.','Copy best.txt to /home/']},ignore_index=True)
...
cp = cp.append({'Command':'cp main.c def.h /home/usr/rapid/','NL Queries':['Copy 2 files main.c and def.h to destination absolute path /home/usr/rapid/',
				'How do I copy main.c and def.h to a given absolute path /home/usr/rapid/?',
					'How do I copy two files to the folder present in /home/usr/rapid/?']},ignore_index=True)

#cp = cp.append({'Command':'cp src /home/usr/rapid/','NL Queries':['Copy directory src to absolute path directory /home/usr/rapid/',
#				'How do I copy a directory in the current folder to a given absolute path /home/usr/rapid/?',
...
cp = cp.append({'Command':'cp -s file.txt file2.txt','NL Queries':['Create symbolic link file2.txt to point to file1.txt.',
			'How to create symbolic link for file1.txt with the name file1.txt with cp command?',
				'Create symbolic link file2.txt for file1.txt with cp command.']},ignore_index=True)

cp = cp.append({'Command':'cp -s ~/myfiles/file*.txt ~/myfiles2','NL Queries':['Create a symbolic link of multiple files starting with \'file\' in ~/myfiles directory.',
		'How to create symbolic links for multiple files starting with \'file\' in ~/myfiles folder?']},ignore_index=True)
...
#Incomplete
cp = cp.append({'Command':'cp -p -v /etc/resolv.conf $HOME','NL Queries':['Copy all attributes along with the file /etc/resolv.conf to home.',
		'How do I copy /etc/resolve.conf file to home directory along with attributes?',
			'Command to copy /etc/resolv.conf to home along with the attributes.']},ignore_index=True)
...
cp = cp.append({'Command':'cp -l file_4.txt /home/pungki/office','NL Queries':['How do I create a hard link of file file4.txt to /home/pungki/office directory?',
		'How do I create shortcuts for a file file4.txt in /home/pungki/office?',
...
cp = cp.append({'Command':'cp -P file_6.txt ./movie','NL Queries':['How do I copy file file_6.txt with symbolic link as it is to ./movie?',
		'Copy file file_6.txt with symbolic link as it is into another file ./movie.']},ignore_index=True)

cp = cp.append({'Command':'cp -L file_6.txt ./movie','NL Queries':['How do I get the original source file file_6.txt from symbolic link movie?',
...
cp = cp.append({'Command':'cp -a directory_1/ /home/pungki/office','NL Queries':['How do I copy all files including symbolic links in directory_1 to absolute path /home/pungki/office?',
		'Copy files and folders including symbolic links in directory_1 to office folder in /home/pungki.']},ignore_index=True)

cp = cp.append({'Command':'cp --backup=simple -v *.txt ../office','NL Queries':['Create a backup of each existing destination file in office folder in parent directory.',
				'How to create a backup for existing destination files in relative path ../office?',
					'How to keep the existing files instead of overwriting while copying in office folder in parent directory?']},ignore_index=True)
