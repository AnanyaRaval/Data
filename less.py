...
#--- Add commands for -g, -G, -h, -i, -I, -j, -t, -T, -y. }
less = less.append({'Command':'less -f dir1','NL Queries':['Open a directory "dir1" forcefully on command line.',
															'How to force open dir1 directory on command line?',
															'Open folder named dir1 forcefully using less.']},ignore_index = True)

less = less.append({'Command':'less -F list.txt','NL Queries':['Only open list.txt in command line, if it is more than a page wide',
																'Open list.txt with less.If lists.txt has less than one screen of data,quit it automatically.',
																'Display a pageful of lists.txt, if the total content is more than one page.']},ignore_index = True)

less = less.append({'Command':'less -g list.txt','NL Queries':['Open list.txt in command line,one pageful at a time.While searching, highlight the last locations where the query was found in that pageful.',
																'Open list.txt in "less" mode.Highlight only the last locations that match the query.',
																'How to highlight only the last matching items found while seaching the file = list.txt in command line.']},ignore_index = True)
