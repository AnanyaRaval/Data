...
export = export.append({'Command':'export -p','NL Queries':['Show all bash environment variables.','How do I print all environment variables?',
												'View all variables in shell environment.','Display all variables in bash envirnment.']},ignore_index = True)
...
export = export.append({'Command':'export PS1=\'\[\e[1;32m\][\u@\h \W]\$\[\e[0m\] \'','NL Queries':['How do I change the prompt in command line to include current folder name?','Includes the current directory name in the command line prompt.','Command line prompt changes to accomodate the directory name as well.']},ignore_index=True)
export = export.append({'Command':'export FOO','NL Queries':['Allowes usage of variable FOO in the child processes.','How do I use the variable FOO in my child processes?']},ignore_index=True)
export = export.append({'Command':'export -f function_name','NL Queries':['How do I export a shell function?','Allows the usage of function function_name to all the child processes.','Exxports a shell function to child processes.']},ignore_index=True)
